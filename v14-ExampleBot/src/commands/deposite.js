...
module.exports.run = async (client, message, args) => {
  ...
  if (result.error) {
    if (result.type === "money")
      return message.reply("Specify an amount to deposite");
    if (result.type === "negative-money")
      return message.reply("You can't deposite negative money");
    if (result.type === "low-money")
      return message.reply("You don't have that much money in wallet.");
    if (result.type === "no-money")
      return message.reply("You don't have any money to deposite");
    if (result.type === "bank-full")
      return message.reply("Your bank is full. It has reached it's limit.");
  } else {
